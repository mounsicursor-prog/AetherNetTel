...
## OperationBrowser (UI type Chrome)

Ce template inclut une UI “type Chrome” (barre d’adresse + back/forward/reload/home) qui charge les pages *uniquement* dans un `iframe` via Ultraviolet.
...
- Ouvre ensuite l’URL affichée dans la console (si `8080` est occupé, le serveur essaie `8081`, `8082`, etc.).
- Utilise **Transport** pour basculer entre **Epoxy (wisp)** et **Bare**.
